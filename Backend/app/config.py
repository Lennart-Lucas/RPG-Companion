...
class Settings(BaseSettings):
    ...
    database_url: str = (
        "postgresql+asyncpg://rpg_companion:rpg_companion@localhost:5434/rpg_companion"
    )
    database_url_sync: str = (
        "postgresql+psycopg2://rpg_companion:rpg_companion@localhost:5434/rpg_companion"
    )
    ...
    cors_origins: str = "http://localhost:3000,http://localhost:8010"
...
def _database_url_for_host(url: str) -> str:
    if _running_in_docker():
        return (
            url.replace("@localhost:5434", "@db:5432")
            .replace("@127.0.0.1:5434", "@db:5432")
            .replace("@localhost:5432", "@db:5432")
            .replace("@127.0.0.1:5432", "@db:5432")
        )
    if "@db:" in url:
        return url.replace("@db:", "@localhost:").replace(":5432/", ":5434/")
    return url
