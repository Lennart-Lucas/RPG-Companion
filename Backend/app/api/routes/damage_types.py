...
async def create_damage_type(
    ...
    await session.flush()
    await session.refresh(damage_type)
    return DamageTypeResponse.model_validate(damage_type)
...
async def list_damage_types(
    ...
    return DamageTypeListResponse(
        items=[DamageTypeResponse.model_validate(item) for item in items],
        total=total,
        limit=limit,
        offset=offset,
    )
...
async def delete_damage_type(
    ...
    await damage_type_service.delete_damage_type(session, user, damage_type_id)
